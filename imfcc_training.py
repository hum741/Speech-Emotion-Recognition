...
# Author: hum741
# CreatTime: 2021/10/4
# FileName: imfcc_training
# Description:
#!/usr/bin/env python
#-*- conding:utf-8 -*-
# Author: hum741
# CreatTime: 2021/9/19
# FileName: MFCC_training
# Description: 基于二维卷积网络对IMFCC特征进行训练
...
def smooth_labels(labels, factor=0.01):#标签平滑
    ...
    return labels
#调用函数读取数据
mfcc_data = load_f(r'mfcc_feature.pkl')#属性为list
imfcc_data = load_f(r'imfcc_feature.pkl')
labels = load_f(r'label.pkl')
mfcc_data = Normalization(mfcc_data)
imfcc_data = Normalization(imfcc_data)
#分割训练集测试集
x_train, y_train, x_val, y_val = Train_Val_Split(imfcc_data,labels)
# y_train = smooth_labels(y_train)
#*****************************模型训练*****************************
#**********定义模型**********
model = models.Sequential()#构建sequential顺序模型
#第一层
model.add(layers.Conv2D(input_shape=(39,126,1),filters=32, kernel_size=(3,3),strides=2, padding='valid',activation='relu',data_format='channels_last',name='CONV1'))
model.add(layers.Dropout(0.3,name='DP1'))
model.add(layers.Conv2D(filters=64, kernel_size=(3,3), strides=2, padding='valid', activation='relu', kernel_regularizer=regularizers.l2(0.0015),name='CONV2'))
model.add(layers.Dropout(0.3,name='DP2'))#防止过拟合，随机去掉20%的神经元连接
model.add(layers.Conv2D(filters=128, kernel_size=3, strides=2,activation='relu', padding='valid', kernel_regularizer=regularizers.l2(0.0015),name='CONV3'))
model.add(layers.Dropout(0.3,name='DP3'))#防止过拟合，随机去掉20%的神经元连接
model.add(layers.Conv2D(filters=256, kernel_size=3, strides=2, activation='relu', padding='valid',kernel_regularizer=regularizers.l2(0.0015),name='CONV4'))
model.add(layers.Dropout(0.3,name='DP4'))#防止过拟合，随机去掉20%的神经元连接
...
#**********编译模型**********
#配置学习过程 （多分类问题）
opt = optimizers.RMSprop(lr=0.00001, decay=1e-5)#定义RMS优化器，用于优化损失函数
model.compile(loss='categorical_crossentropy', optimizer=opt,metrics=['accuracy'])#定义损失函数ADAMval

callbacks_list = [keras.callbacks.EarlyStopping(monitor='accuracy',patience=100,verbose=0,mode='auto')]#提前停止
...
history = model.fit(x_train, y_train,epochs=2000,batch_size=64,validation_data=(x_val, y_val),shuffle=True,callbacks=callbacks_list)

model.save('imfcc_model.h5')
...
ax1.set_title('imfcc model loss')
ax1.set_ylabel('loss')
