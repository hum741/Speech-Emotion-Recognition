...
# FileName: mel_spectrum_training
# Description:训练mel谱图特征
...
def smooth_labels(labels, factor=0.2):#标签平滑
    # smooth the labels
    labels *= (1 - factor)
    ...
    return labels
#调用函数读取数据
spectrum_data = load_f(r'mel_spectrum_feature.pkl')#属性为list
...
#第一层
model.add(layers.Conv2D(input_shape=(70,123,1),filters=32, kernel_size=3,strides=2, padding='same',activation='relu',data_format='channels_last',name='CONV1'))
model.add(layers.Dropout(0.5,name='DP1'))
model.add(layers.Conv2D(filters=64, kernel_size=3, strides=2, activation='relu', padding='same',kernel_regularizer=regularizers.l2(0.002),name='CONV2'))
model.add(layers.Dropout(0.5,name='DP2'))#防止过拟合，随机去掉20%的神经元连接
model.add(layers.Conv2D(filters=128, kernel_size=3, strides=2,activation='relu', padding='same', kernel_regularizer=regularizers.l2(0.002),name='CONV3'))
model.add(layers.Dropout(0.5,name='DP3'))#防止过拟合，随机去掉20%的神经元连接
model.add(layers.Conv2D(filters=256, kernel_size=3, strides=2, activation='relu', padding='same',kernel_regularizer=regularizers.l2(0.002),name='CONV4'))
model.add(layers.Dropout(0.5,name='DP4'))#防止过拟合，随机去掉20%的神经元连接
model.add(layers.Conv2D(filters=512, kernel_size=3, strides=2, activation='relu', padding='same',kernel_regularizer=regularizers.l2(0.002),name='CONV5'))
...
keras.callbacks.ModelCheckpoint(filepath='mel_spectrum_model_checkpoint.h5',monitor='val_loss',save_best_only=True)#该回调函数将在每个epoch后保存模型到filepath
...
model.save('mel_spectrum_model.h5')
...
ax2.set_title('mel spectrum model acc')
ax2.set_ylabel('acc')
