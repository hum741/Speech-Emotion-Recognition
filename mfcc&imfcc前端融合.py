...
# FileName: mfcc&imfcc前端融合
# Description:将MFCC和IMFCC拼接后训练
...
#分割训练集测试集
x_train, y_train, x_val, y_val,x_test,y_test = Train_Val_Split(mfcc_imfcc0,labels)
y_train = smooth_labels(y_train)
#*****************************模型训练*****************************
#**********定义模型**********
model = models.Sequential()#构建sequential顺序模型
#第一层
model.add(layers.Conv2D(input_shape=(78,126,1),filters=32, kernel_size=(3,3),strides=2, padding='valid',activation='relu',data_format='channels_last',name='CONV1'))
...
ax1.set_title('mfcc imfcc concatenate model')
